...
App **Expo SDK 57** (React 19, React Native 0.86, TypeScript 6) para o gerenciador de tarefas Conecthus. Telas nativas com **React Navigation**, **axios**, **AsyncStorage** e **MQTT over WebSocket** em tempo real, espelhando o fluxo do frontend web.
...
# ou local: cd backend && npm run start:dev (requer Postgres/Redis/Mosquitto)
...
- **Auth**: `AuthContext` carrega sessão do `AsyncStorage` no mount (`loadSession` → `setToken` em memória + `setUser`). `login`/`register` chamam `/auth/*`, persistem via `persistSession` e atualizam o token em memória (usado pelo interceptor do axios). `logout` chama `removeSession`.
- **Tasks**: `TasksScreen` busca via `listTasks` (filtros `status`/`search`, paginação), `FlatList` + `TaskFilters` + `NotificationOverlay` (MQTT). Mudanças via MQTT disparam `TASKS_CHANGED_EVENT` (DeviceEventEmitter) que recarrega a lista.
- **Notificações**: `useNotifications(userId)` conecta em `MQTT_URL` (`mqtt.connect`), assina `notifications/{userId}`, parseia payload JSON e expõe `notifications` + `dismiss`.
