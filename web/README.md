...
Single-origin SPA for the Conecthus task manager. Built with **React 19**, **Vite 8** (TypeScript), **react-router**, **axios**, **Redux Toolkit** and **MQTT over WebSocket** for real-time notifications.
...
├── auth/
│   └── AuthContext.tsx      # AuthContext bridge + session integration
├── features/
│   ├── auth/                # Auth slice + selectors
│   ├── tasks/               # Tasks slice + selectors
│   └── notifications/      # Notifications slice + selectors/hook
